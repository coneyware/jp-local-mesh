...
export const MESH_INFO: MeshInfo[] = [
	{
		"name": "一次メッシュ"
		, "kind": 1
		, "width": "80km"
		, "converter": (mesh:string) => mesh.substring(0, 4)
		, "check": (mesh:string) => kind1.test(mesh)
	}
	, {
		"name": "二次メッシュ"
		, "kind": 2
		, "width": "10km"
		, "converter": (mesh:string) => {
			if (mesh.length < 6) {
				throw new Error(`Mesh Code convert ng: ${mesh} to 10km mesh.`);
			}
			return mesh.substring(0, 6);
		}
		, "check": (mesh:string) => kind2.test(mesh)
	}
	, {
		"name": "５倍地域メッシュ"
		, "kind": 2.5
		...
		, "check": (mesh:string) => kind25.test(mesh)
	}
	, {
		"name": "２倍地域メッシュ"
		, "kind": 2.2
		...
		, "check": (mesh:string) => kind22.test(mesh)
	}
	, {
		"name": "三次メッシュ"
		, "kind": 3
		...
		, "check": (mesh:string) => kind3.test(mesh)
	}
	, {
		"name": "2分の1地域メッシュ"
		, "kind": 3.5
		...
		, "check": (mesh:string) => kind3p2.test(mesh)
	}
	, {
		"name": "4分の1地域メッシュ"
		, "kind": 3.25
		...
		, "check": (mesh:string) => kind3p4.test(mesh)
	}
	, {
		"name": "8分の1地域メッシュ"
		, "kind": 3.125
		, "width": "125m"
		, "converter": (_mesh:string) => {
			throw new Error("Minimam mesh. convert ng");
		}
		, "check": (mesh:string) => kind3p8.test(mesh)
	}
	, {
		"name": "10分の1地域メッシュ"
		, "kind": 3.1
		...
		, "check": (mesh:string) => kind3p10.test(mesh)
	}
	, {
		"name": "20分の1地域メッシュ"
		, "kind": 3.05
		...
		, "check": (mesh:string) => kind3p20.test(mesh)
	}
];
...
const calcAngle = (
	parentAngle:angleCalc.Angle
	, addAngle:angleCalc.Angle
	, angleNum:number
) => {
...
export const meshToLatLng = (
	...
	const {
		...
		, "digit": slat2
		, "oppositeDigit": nlat2
	} = calcAngle(slatAngle, lat2add, lat2Source);
	const {
		"angle": wlng2Angle
		, "digit": wlng2
		, "oppositeDigit": elng2
	} = calcAngle(angleCalc.toAngle(wlng), lng2add, lng2Source);
	...
	if (checkMeshInfo.kind === 2.2) {
		...
		const {
			"digit": slat2x
			, "oppositeDigit": nlat2x
		} = calcAngle(slat2Angle, lat2xadd, lat2xSource / 2);
		const {
			"digit": wlng2x
			, "oppositeDigit": elng2x
		} = calcAngle(wlng2Angle, lng2xadd, lng2xSource / 2);
	...
	if (checkMeshInfo.kind === 2.5) {
		...
		const {
			"digit": slat5x
			, "oppositeDigit": nlat5x
		} = calcAngle(slat2Angle, lat5xadd, Math.floor(source5x / 3));
		const {
			"digit": wlng5x
			, "oppositeDigit": elng5x
		} = calcAngle(wlng2Angle, lng5xadd, (source5x + 1) % 2);
	...
	const {
		...
		, "digit": slat3
		, "oppositeDigit": nlat3
	} = calcAngle(slat2Angle, lat3add, lat3Source);
	const {
		"angle": wlng3Angle
		, "digit": wlng3
		, "oppositeDigit": elng3
	} = calcAngle(wlng2Angle, lng3add, lng3Source);
	...
	if (isExtend) {
		...
		const {
			"angle": slat1p10Angle
			, "digit": slat1p10
			, "oppositeDigit": nlat1p10
		} = calcAngle(slat3Angle, lat1p10add, lat1p10Source);
		const {
			"angle": wlng1p10Angle
			, "digit": wlng1p10
			, "oppositeDigit": elng1p10
		} = calcAngle(wlng3Angle, lng1p10add, lng1p10Source);
		...
		const {
			// "angle": slat1p20Angle
			"digit": slat1p20
			, "oppositeDigit": nlat1p20
		} = calcAngle(slat1p10Angle, lat1p20add, Math.floor(source1p20 / 3));
		const {
			// "angle": wlng1p20Angle
			"digit": wlng1p20
			, "oppositeDigit": elng1p20
		} = calcAngle(wlng1p10Angle, lng1p20add, (source1p20 + 1) % 2);
	...
	const {
		"angle": slat1p2Angle
		, "digit": slat1p2
		, "oppositeDigit": nlat1p2
	} = calcAngle(slat3Angle, lat1p2add, Math.floor(source1p2 / 3));
	const {
		"angle": wlng1p2Angle
		, "digit": wlng1p2
		, "oppositeDigit": elng1p2
	} = calcAngle(wlng3Angle, lng1p2add, (source1p2 + 1) % 2);
	...
	const {
		"angle": slat1p4Angle
		, "digit": slat1p4
		, "oppositeDigit": nlat1p4
	} = calcAngle(slat1p2Angle, lat1p4add, Math.floor(source1p4 / 3));
	const {
		"angle": wlng1p4Angle
		, "digit": wlng1p4
		, "oppositeDigit": elng1p4
	} = calcAngle(wlng1p2Angle, lng1p4add, (source1p4 + 1) % 2);
	...
	const {
		// "angle": slat1p8Angle
		"digit": slat1p8
		, "oppositeDigit": nlat1p8
	} = calcAngle(slat1p4Angle, lat1p8add, Math.floor(source1p8 / 3));
	const {
		// "angle": wlng1p8Angle
		"digit": wlng1p8
		, "oppositeDigit": elng1p8
	} = calcAngle(wlng1p4Angle, lng1p8add, (source1p8 + 1) % 2);
