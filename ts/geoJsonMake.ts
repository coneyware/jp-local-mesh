...
const PREFECTURE_CODE_NAMES: {[key: string]: string} = {
	"01": "北海道"
	, "02": "青森県"
	, "03": "岩手県"
	, "04": "宮城県"
...
export const fetchTextAsync = async(url: string, option: object = {}, encoding = "utf-8") => {
...
type meshCsv = {
	...
	, 都道府県市区町村コード: string
	, 市区町村名: string
	, 基準メッシュコード: string | undefined
	, ["基準メッシュ・コード"]: string
}

const meshcodeToGeojsonAsync = async(
	baseDir:string
	, outputPath:string
	, codes:string[]
	...
	try {
		...
		const toJson:{[key: string]: Feature<Geometry, GeoJsonProperties>[] | unknown} = {
			"type": "FeatureCollection"
			, features
 ...
 * @memberof meshToGeojson
 */
const meshToGeojsonAsync = async(
	baseDir:string
	, outputDir:string
	, meshs:string[]
	, kind:number
...
const makeBaseDir = (prefecture:string, workDir:string|null = null) => {
	let baseDir = "";
	if (workDir === null) {
 ...
 */
const meshCsvDownloadAsync = async(prefecture:string, baseDir:string): Promise<meshCsv[]> => {
	...
	return parse(meshlistCsv, parserOptions) as meshCsv[];
};

export const getPrefectures = (): [string, string][] => {
...
export const getMunicipalitiesAsync = async(prefecture:string, workDir:string|null = null) => {
 ...
 */
export const makeAsync = async(commandlineOptions: MeshMakeOptions): Promise<MeshFileInfo[]> => {
	// let baseDir = "";
